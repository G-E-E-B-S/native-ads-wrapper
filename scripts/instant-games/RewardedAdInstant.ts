...
export class RewardedAdInstant implements IAdCollectionDelegate, IRewardedAd {
    private TAG = "RewardedAdControllerInstant";
    private delegate: IRewardedAdDelegate;
    private adController: IRewardedAdController;
    ...
    init(adController: IRewardedAdController) {
    ...
    private initAdUnits() {
        if (!this.isVideoRewardSupported()) {
            console.log(this.TAG + " - FBInstant getRewardedVideoAsync Api not supported");
            this.dependency.logAnalyticsEvent(
                AdAnalyticsEvents.ApiError, {
                "kingdom": "api_not_supported",
                "phylum": "getRewardedVideoAsync"
            });
            return;
        }
    ...
    private initAdUnit(adType: string, placementId: string) {
        ...
        if (!adUnit) {
            adUnit = new AdUnit(placementId, this, AdType.Rewarded, this.dependency);
    ...
    private isVideoRewardSupported(): boolean {
        return FBInstantAdsSafe.isApiSupported("getRewardedVideoAsync");
    }
